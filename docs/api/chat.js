// Vercel serverless function to proxy AI API requests
// Supports both Gemini and Hugging Face APIs
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (useHuggingFace) {
      ...
      return await handleGeminiRequest(req, res, message, conversationHistory);
    }
...
- Always maintain scientific accuracy and intellectual rigor`;
  ...
  while (attempts < maxRetries) {
    ...
    attempts++;
    console.log(`[${new Date().toISOString()}] Hugging Face attempt ${attempts} failed with ${response.status}, retrying...`);
  ...
  res.status(200).json({ 
    ...
    usage: {
      ...
      outputTokens: data.usage?.generated_tokens || 0,
      totalTokens: (data.usage?.prompt_tokens || 0) + (data.usage?.generated_tokens || 0)
    }
  });
}

async function handleGeminiRequest(req, res, message, conversationHistory) {
