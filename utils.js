...
export function updateDebugInfo() {
    ...
    if (currentTime - debugInfo.lastFpsUpdate >= 500) {
        const deltaTime = currentTime - debugInfo.lastFpsUpdate;
        debugInfo.fps = Math.round((debugInfo.frameCount * 1000) / deltaTime);
        debugInfo.frameCount = 0;
        debugInfo.lastFpsUpdate = currentTime;
    }
    
    debugInfo.lastTime = currentTime;
}

// Draw debug info in top left corner
export function drawDebugInfo(ctx, world, hangingTexts, backgroundParticles, render) {
    ctx.save();
    ...
    const blackSparks = world.bodies.filter(body => body.label === 'spark' && body.render?.fillStyle === 'black').length;
    const firstText = hangingTexts.length > 0 ? hangingTexts[0] : null;
    const bgColor = document.body.style.backgroundColor || 'default';
    const renderBg = render.options.background || 'default';
    
    const debugText = [
        `FPS: ${debugInfo.fps}`,
        `Hanging Texts: ${hangingTexts.length}`,
        `Background Particles: ${backgroundParticles ? backgroundParticles.particles.length : 0}`,
        `Sparks: ${sparkCount} (${blackSparks} black)`,
        `Physics Bodies: ${world.bodies.length}`,
        `Constraints: ${world.constraints.length}`,
        `Mouse: ${Math.round(render.mouse.position.x)}, ${Math.round(render.mouse.position.y)}`,
        `--- COLOR DEBUG ---`,
        `Body BG: ${bgColor}`,
        `Render BG: ${renderBg}`,
        `Text Color: ${firstText?.options?.color || 'N/A'}`,
        `String Color: ${firstText?.options?.stringColor || 'N/A'}`,
        `Spark Color: ${firstText?.options?.sparkColor || 'N/A'}`,
        `BG Particles: ${backgroundParticles?.colors?.[0] || 'N/A'}`
    ];
    ...
    const swatchY = 20 + (debugText.length * 14) + 10;
    ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
    ctx.fillRect(15, swatchY, 250, 30);
    
    ctx.strokeStyle = 'rgba(0, 0, 0, 1)';
    ctx.lineWidth = 1;
    ctx.strokeRect(15, swatchY, 250, 30);
    
    // Color swatches with labels
    const swatches = [
        { label: 'Text', color: firstText?.options?.color || 'rgba(255, 215, 0, 1)', x: 25 },
        { label: 'String', color: firstText?.options?.stringColor || 'rgba(255, 215, 0, 1)', x: 75 },
        { label: 'Spark', color: firstText?.options?.sparkColor || 'rgba(255, 215, 0, 1)', x: 125 },
        { label: 'BG Part', color: backgroundParticles?.colors?.[0] || 'rgba(255, 215, 0, 1)', x: 175 },
        { label: 'Body BG', color: bgColor === 'white' ? 'white' : (bgColor || 'transparent'), x: 225 }
    ];
    
    swatches.forEach(swatch => {
        // Draw color swatch
        ctx.fillStyle = swatch.color;
        ctx.fillRect(swatch.x, swatchY + 5, 15, 15);
        
        // Draw border around swatch
        ctx.strokeStyle = 'rgba(0, 0, 0, 0.5)';
        ctx.lineWidth = 1;
        ctx.strokeRect(swatch.x, swatchY + 5, 15, 15);
        
        // Draw label
        ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
        ctx.font = '10px monospace';
        ctx.textAlign = 'center';
        ctx.fillText(swatch.label, swatch.x + 7.5, swatchY + 27);
    });
